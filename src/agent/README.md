...
The agent Dockerfile must be built with the repository root as its context,
because it uses files from both `src/agent` and `src/shared`:

```sh
docker build -f src/agent/Dockerfile -t agent .
```

The Dockerfile sets `PYTHONPATH=/app/src/agent` and places the shared tools in
the agent package so that the `src.agents` imports resolve inside the image.
You can verify the image before starting the service:

```sh
docker run --rm --entrypoint python agent \
  -c "from src.agents.agents import Agent; print(Agent)"
```
